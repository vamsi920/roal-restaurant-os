...
export async function saveRestaurantProfileSettingsAction(
  ...
  if (parsed.data.knowledge_entries !== undefined) {
    await replaceRestaurantKnowledgeEntries(supabase, {
      restaurantId,
      organizationId: access.access.restaurant.organization_id,
      entries: parsed.data.knowledge_entries,
    });
  } else if (parsed.data.knowledge_base_text !== undefined) {
    await replaceRestaurantKnowledgeEntries(supabase, {
      restaurantId,
      organizationId: access.access.restaurant.organization_id,
  ...
  if (parsed.data.upsell_entries !== undefined) {
    await replaceRestaurantUpsellRules(supabase, {
      restaurantId,
      organizationId: access.access.restaurant.organization_id,
      rules: parsed.data.upsell_entries,
    });
  } else if (parsed.data.upsell_rules_text !== undefined) {
    await replaceRestaurantUpsellRules(supabase, {
      restaurantId,
      organizationId: access.access.restaurant.organization_id,
  ...
  const knowledgeTouched =
    parsed.data.knowledge_entries !== undefined ||
    parsed.data.knowledge_base_text !== undefined;
  const upsellTouched =
    parsed.data.upsell_entries !== undefined ||
    parsed.data.upsell_rules_text !== undefined;

  if (upsellTouched) {
    afterRestaurantUpsellMutation(restaurantId, {
      userId: access.context.user.id,
      restaurantName: parsed.data.name,
    });
  } else if (knowledgeTouched) {
    afterRestaurantKnowledgeMutation(restaurantId, {
      userId: access.context.user.id,
      restaurantName: parsed.data.name,
    });
  } else {
