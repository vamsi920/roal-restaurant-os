import { groupKey } from "@/lib/menu-editor/modifier-groups";
import type { DbItem, DbModifier } from "@/lib/types";
import type { AgentToolErrorBody } from "@/lib/agent-tools/schemas";
...
export type CartValidationOptions = {
  /** sync_draft_order may persist an empty cart; finalize must not. */
  allowEmpty?: boolean;
  /** When true, unavailable items fail validation (default true). */
...
type MenuIndex = {
  ...
  modifiersByItemId: Map<string, DbModifier[]>;
};
...
function buildMenuIndex(items: DbItem[], modifiers: DbModifier[]): MenuIndex {
  ...
  const modifiersByItemId = new Map<string, DbModifier[]>();
...
function resolveItem(
  ...
  index: MenuIndex,
  lineIndex: number
): { item: DbItem | null; issues: CartValidationIssue[] } {
...
function validateModifierGroups(
  ...
  const groups = new Map<string, { label: string; mods: DbModifier[] }>();
...
export function validateCartAgainstMenu(
  ...
  const rejectUnavailable = options.rejectUnavailable ?? true;
          ...
          issues: [
            {
              ...
              suggestion:
                "Add items from get_menu_items before sync_draft_order or finalize_order.",
            },
          ],
          normalizedItems: [],
        };
  ...
  items.forEach((raw, i) => {
    ...
    if (rejectUnavailable && !item.is_available) {
      issues.push({
        index: i,
...
export function validateCartForSync(
  ...
  return validateCartAgainstMenu(items, menuItems, menuModifiers, {
    allowEmpty: false,
    rejectUnavailable: true,
  });
}

/** @deprecated Use validateCartForSync */
export function validateCartAgainstMenuAllowEmpty(
  items: unknown,
  menuItems: DbItem[],
  menuModifiers: DbModifier[]
): CartValidationResult {
  return validateCartForSync(items, menuItems, menuModifiers);
}
...
export function formatCartValidationError(
  ...
  context: {
    tool: "sync_draft_order" | "finalize_order" | "compute_totals";
  }
): AgentToolErrorBody {
  ...
  if (emptyCart) {
    ...
    recovery_hint =
      "Remove sold-out items and substitute available menu options before syncing or finalizing.";
  } else if (stale) {
    recovery_hint =
      "Call get_menu_items immediately, then replace stale item_id or name values.";
  } else if (context.tool === "finalize_order") {
    recovery_hint =
      "Fix cart lines, then call sync_draft_order before finalize_order with the same session_id.";
  } else if (context.tool === "compute_totals") {
    recovery_hint =
      "Use exact item names or item_id values from the live menu, with available items only.";
  }

  return {
    error: "order_validation_failed",
    code: "order_validation_failed",
    message: `${context.tool} failed menu validation.`,
...
/** Persist deterministic line shape on draft_orders / receipts. */
