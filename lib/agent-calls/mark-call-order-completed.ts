...
export function buildOrderCompletedCallEventRow(
  ...
  const isHarness = isTestHarnessBillingSession(sessionId);
...
/** Clear in-progress call state when an order is finalized with a receipt. */
